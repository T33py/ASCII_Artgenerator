...
def mutate(letters: list, swap: int, dist = 1):
    ...
    for val in range(dist):
        ...
        letter2 = letters[pivot]

        if pivot + val < d_down and pivot - val < 0:
            letter1 = letters[pivot - val]
            letter2 = letters[pivot + val]

        elif pivot + val < d_down:
            letter1 = letters[pivot - val]
            letter2 = letters[0]

        elif pivot - val < 0:
            letter1 = letters[len(letters) - 1]
            letter2 = letters[pivot + val]
         

        letters[letters.index(letter1)] = letter2
        letters[letters.index(letter2)] = letter1
    return letters
